/**
 * Cria (ou atualiza) um tenant de escritório de advocacia.
 *
 * Não é o bootstrap do dono da instância: este usuário é admin da organização,
 * sem `platform_admins`. O quadro de e-commerce que o gatilho semeia é trocado
 * pelos funis do perfil (`lib/onboarding/perfil-advocacia.ts`).
 *
 * Credenciais vêm do ambiente — nunca deste arquivo:
 *
 *   OWNER_EMAIL=socio@escritorio.com OWNER_PASSWORD='…' \
 *   OWNER_ORG_NAME='Rodrigues Advogados' \
 *   npx tsx scripts/provisionar-escritorio-advocacia.ts
 *
 * Para entrar numa organização que já existe (e já tem WhatsApp):
 *   OWNER_ORG_ID=<uuid> ...
 *
 * Idempotente: usuário existente tem a senha atualizada; org existente pelo
 * slug recebe o perfil de novo; associação admin é garantida.
 */
...
const ORG_NAME = env.OWNER_ORG_NAME || "Escritório de Advocacia";
...
async function marcarOnboarded(orgId: string): Promise<void> {
  const { error } = await admin
    ...
    .update({
      ...
      onboarding_state: {
        welcome: {
          o_que_faz: "Escritório de advocacia",
          display_name: ORG_NAME,
          timezone: "America/Sao_Paulo",
        },
...
async function main(): Promise<void> {
  ...
  const perfil = await aplicarPerfilAdvocacia(admin, orgId);
  ...
  console.log(
    [
      ...
      `  usuário: ${EMAIL}`,
      `  org:     ${orgId}`,
      `  funil:   novos clientes ${perfil.pipelineNovosClientesId}`,
      `  funil:   processos ${perfil.pipelineProcessosId}`,
      `  agenda:  ${perfil.tiposDeAgendaCriados} tipo(s) novo(s)`,
...
main().catch((err) => {
  console.error("❌ Provisionar escritório falhou:", err);
  process.exit(1);
});
