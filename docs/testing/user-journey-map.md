...
| J1.22 | Convidado que **ainda não tem conta** | a tela de aceite oferece "Ainda não tenho conta", o signup recebe o convite, não pede nome de empresa e trava o e-mail; ao criar a senha a conta **já nasce confirmada** e vai para o aceite — o clique no convite prova o e-mail, e um segundo e-mail do GoTrue travava a pessoa quando o correio do Auth não entregava. Continua sem ganhar organização própria · **PASS** (`lib/auth/criar-conta-convidada.test.ts`, `app/actions/auth/signUp.test.ts`, `lib/auth/convite-no-signup.test.ts`, `tests/e2e/invite-lifecycle.spec.ts` casos 10–13) |
