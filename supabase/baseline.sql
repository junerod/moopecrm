...
begin
  ...
  -- 6b. campaign_recipients — phone copiado no envio. Status e timestamps ficam.
...
alter table job_queue add constraint job_queue_kind_check
  ...
  check (kind in ('inbound_turn','followup_turn','watchdog','flywheel','case_reply_turn','operator_turn','copilot_turn'));
...
alter table job_queue add constraint job_queue_turn_needs_contact
  check ((kind in ('inbound_turn','followup_turn','case_reply_turn','operator_turn','copilot_turn')) = (contact_id is not null));
...
-- apêndice NÃO reconstrói a constraint — ver
-- tests/unit/baseline-constraint-reconstruida.test.ts.
...
-- ---- papel do contato e temperatura do lead (migration 0203) ----
alter table public.contacts
...
-- ---- próxima ação: alerta do atendente + ledger (migration 0204) ----
alter table public.user_organizations
...
comment on table public.demanda_alert_deliveries is
  'Ledger de alerta INTERNO ao atendente. unique (org, demanda, kind, scheduled_for) = no máximo uma entrega lógica.';

-- ---- campanha comercial mínima (migration 0205) ----
...
-- ---- campanha premium: settings + canal por destinatário (migration 0209) ----
...
drop index if exists public.campaign_recipients_unique;

create unique index if not exists campaign_recipients_unique
  on public.campaign_recipients (campaign_id, contact_id, channel);

comment on column public.campaigns.settings is
  'Objetivo, canais, anexos (path no Storage), preparing. Sem base64. Migration 0209.';
comment on column public.campaign_recipients.destination is
  'Telefone ou e-mail efetivamente usados. Redigido na LGPD. Migration 0209.';

-- destination entra na cascata LGPD (mesmo corpo da 0206 + destination)
