...
describe("extrairRegua — os pares saem do globals.css, nunca de lista à mão", () => {
  it("acha os dois temas, a rampa do produto e os neutros", () => {
    ...
    expect(REGUA.escuro.neutros[9]).toBe("#0b1220");
  ...
  it("reproduz as razões medidas à mão no design system", () => {
    ...
    expect(razao("--color-accent", "--color-bg")).toBeCloseTo(5.44, 2);
    expect(razao(":focus-visible/outline", "--color-bg")).toBeCloseTo(3.75, 2);
...
describe("marca acromática — o accent do produto permanece", () => {
  ...
  it("o accent que permanece é cromático e separável do neutro do mesmo grau", () => {
    ...
    // Superfícies MOOPE (slate frio): Sage × neutral do mesmo grau agora mede
    // 0,0864 no claro e 0,2294 no escuro. O piso do produto continua 0,05 —
    // o briefing de 0,08 caberia no claro de hoje, e não cabia no greige.
    expect(separacaoDoNeutro(REGUA.claro, marca.claro.grauDoAccent, marca.claro.accent)).toBeCloseTo(0.0864, 4);
    expect(separacaoDoNeutro(REGUA.escuro, marca.escuro.grauDoAccent, marca.escuro.accent)).toBeCloseTo(0.2294, 4);
